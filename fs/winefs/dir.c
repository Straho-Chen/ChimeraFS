...
static int winefs_add_dirent_to_buf(winefs_transaction_t *trans,
				    ...
				    struct winefs_direntry *de, u8 *blk_base,
...
found_free_slot:
	if (de->ino) {
		...
		winefs_add_logentry(dir->i_sb, trans, &de->ino, sizeof(de->ino),
				    LE_DATA);
	}
		     ...
		     struct inode *inode)
{
	...
	if (block >= 0) {
		...
		retval = winefs_add_dirent_to_buf(trans, dentry, inode, NULL,
						  blk_base, pidir);
		if (retval != -ENOSPC)
			goto out;
	}
	...
	retval = winefs_alloc_blocks_weak(trans, dir, blocks, 1, false, ANY_CPU,
					  0);
	if (retval)
		goto out;
			...
			struct inode *inode)
{
	...
	winefs_add_logentry(sb, trans, &res_entry->ino, sizeof(res_entry->ino),
			    LE_DATA);
	winefs_memunlock_block(sb, blk_base);
	res_entry->ino = 0;
	winefs_memlock_block(sb, blk_base);
