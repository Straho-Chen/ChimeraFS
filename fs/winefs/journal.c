...
static uint32_t winefs_recover_transaction(struct super_block *sb,
...
static uint32_t winefs_process_transaction(struct super_block *sb,
					   uint32_t head, uint32_t tail,
					   winefs_logentry_t *le, bool recover,
					   int *processed)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	winefs_transaction_t trans;
	...
	do {
		...
		if ((new_head == tail) ||
			...
			if (recover) {
				...
				head = new_head;
				if (gen_id == MAX_GEN_ID)
					winefs_invalidate_logentries(sb,
								     &trans);
			}
				...
				int take_lock)
{
	...
	while (head != tail) {
		...
		if (gen_id == le16_to_cpu(le->gen_id)) {
			...
			new_head = winefs_process_transaction(
	...
	if (unmount) {
		...
		if (journal->head != journal->tail)
			winefs_dbg(
				"WINEFS: umount but journal not empty %x:%x\n",
...
int winefs_journal_soft_init(struct super_block *sb)
{
	...
	sbi->journal_mutex = (struct mutex *)kmalloc(
		sizeof(struct mutex) * sbi->cpus, GFP_KERNEL);
	...
	if (sbi->journal_base_addr == NULL) {
		sbi->journal_base_addr = (void **)kmalloc(
			sizeof(void *) * sbi->cpus, GFP_KERNEL);
	}

	for (i = 0; i < sbi->cpus; i++) {
		journal = winefs_get_journal(sb, i);
		sbi->journal_base_addr[i] =
			winefs_get_block(sb, le64_to_cpu(journal->base));
			     ...
			     uint32_t size)
{
	...
	sbi->journal_base_addr =
		(void **)kmalloc(sizeof(void *) * sbi->cpus, GFP_KERNEL);
	for (i = 0; i < sbi->cpus; i++) {
		journal = winefs_get_journal(sb, i);
		...
		sbi->journal_base_addr[i] =
			winefs_get_block(sb, base + (size * i));
...
static int winefs_free_logentries(struct super_block *sb, int max_log_entries,
				  int cpu)
{
	int freed_entries = 0;
					     ...
					     int max_log_entries, int cpu)
{
	...
	trans->transaction_id =
		atomic64_fetch_add(1, &sbi->next_transaction_id);
again:
	...
	if (avail_size < req_size) {
		...
		for (retry = 0; retry < 3; retry++) {
			freed_size += winefs_free_logentries(
				sb, max_log_entries, cpu);
...
int winefs_add_logentry(struct super_block *sb, winefs_transaction_t *trans,
			void *addr, uint16_t size, u8 type)
{
	...
	if ((trans->num_used + num_les) > trans->num_entries) {
		winefs_err(sb,
			   "Log Entry full. tid %lu ne %d tail %x size %x\n",
	...
	if (addr) {
		winefs_dbg_verbose(
			"%s: trans start addr = 0x%p. le = 0x%p. le_start = 0x%llu. num_les = %d. param addr = 0x%p. size = %u. type = %u\n",
			__func__, trans->start_addr, le, le_start, num_les,
			addr, size, type);
	}
...
int winefs_abort_transaction(struct super_block *sb,
			     winefs_transaction_t *trans)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
 ...
 * the same cacheline */
static void winefs_forward_journal(struct super_block *sb,
...
static int winefs_recover_undo_journal(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		while (head != tail) {
			...
			le = (winefs_logentry_t *)(sbi->journal_base_addr[i] +
						   tail);
			if (gen_id == le16_to_cpu(le->gen_id)) {
				tail = winefs_recover_transaction(sb, head,
								  tail, le);
			} else {
				if (gen_id == MAX_GEN_ID) {
					winefs_memunlock_range(sb, le,
							       sizeof(*le));
					invalidate_gen_id(le);
					winefs_memlock_range(sb, le,
							     sizeof(*le));
				}
			}
		}
...
static int winefs_recover_redo_journal(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		while (head != tail) {
			le = (winefs_logentry_t *)(sbi->journal_base_addr +
						   head);
			if (gen_id == le16_to_cpu(le->gen_id)) {
				head = winefs_process_transaction(
					sb, head, tail, le, true, &processed);
			} else {
				if (gen_id == MAX_GEN_ID) {
					winefs_memunlock_range(sb, le,
							       sizeof(*le));
