				   ...
				   char __user *buf, size_t len, loff_t *ppos)
{
	...
	do {
		...
		if (!zero)
			left = __copy_to_user(buf + copied, xip_mem + offset,
					      nr);
		else
...
static ssize_t xip_file_read(struct file *filp, char __user *buf, size_t len,
			     loff_t *ppos)
{
				    ...
				    const char __user *buf, size_t bytes)
{
	...
	copied = bytes -
		 __copy_from_user_inatomic_nocache(kmem + offset, buf, bytes);

	return copied;
}

static ssize_t __winefs_xip_file_write(struct address_space *mapping,
				       ...
				       loff_t pos, loff_t *ppos)
{
	...
	do {
		...
		blocks_found = winefs_get_xip_mem(mapping, index, num_blocks, 1,
						  &xmem, &xpfn);
...
static ssize_t winefs_file_write_fast(struct super_block *sb,
				      struct inode *inode,
				      struct winefs_inode *pi,
				      const char __user *buf, size_t count,
 ...
 * end-of-block
 */
static inline void winefs_copy_to_edge_blk(struct super_block *sb,
					   struct winefs_inode *pi,
					   bool over_blk, unsigned long block,
					   ...
					   void *buf)
{
	...
	if (over_blk) {
		blknr = block >>
			(winefs_inode_blk_shift(pi) - sb->s_blocksize_bits);
		__winefs_find_data_blocks(sb, pi, blknr, &bp, 1);
		ptr = winefs_get_block(sb, bp);
		if (ptr != NULL) {
			if (is_end_blk) {
				ptr = ptr + blk_off - (blk_off % 8);
				count = winefs_inode_blk_size(pi) - blk_off +
					(blk_off % 8);
			} else
				count = blk_off + (8 - (blk_off % 8));

			winefs_memunlock_range(sb, ptr,
					       winefs_inode_blk_size(pi));
			memcpy_to_nvmm(ptr, 0, buf, count);
			winefs_memlock_range(sb, ptr,
					     winefs_inode_blk_size(pi));
		}
	}
}
 ...
 * end-of-block
 */
static inline void winefs_copy_from_edge_blk(struct super_block *sb,
					     struct winefs_inode *pi,
					     bool over_blk, unsigned long block,
					     size_t blk_off, bool is_end_blk,
					     void **buf)
 ...
 * end-of-block
 */
static inline void winefs_clear_edge_blk(struct super_block *sb,
					 struct winefs_inode *pi, bool new_blk,
				  ...
				  size_t len, loff_t *ppos)
{
	...
	same_block =
		(((count + offset - 1) >> winefs_inode_blk_shift(pi)) == 0) ?
			1 :
			0;
	if (block && same_block) {
	...
	trans = winefs_new_transaction(sb, MAX_INODE_LENTRIES + max_logentries,
				       winefs_get_cpuid(sb));
	if (IS_ERR(trans)) {
		ret = PTR_ERR(trans);
		goto out;
	...
	winefs_copy_from_edge_blk(sb, pi, over_sblk, start_blk, offset, false,
				  &start_buf);
	winefs_copy_from_edge_blk(sb, pi, over_eblk, end_blk, eblk_offset, true,
				  &end_buf);

	inplace_blk_list =
		(__le64 *)kmalloc(num_blocks * sizeof(__le64), GFP_KERNEL);
	free_blk_list =
		(__le64 *)kmalloc(num_blocks * sizeof(__le64), GFP_KERNEL);
	log_entries =
		(__le64 **)kmalloc(num_blocks * sizeof(__le64), GFP_KERNEL);
	...
	winefs_alloc_blocks(trans, inode, start_blk, num_blocks, false, ANY_CPU,
			    1, inplace_blk_list, &num_inplace_blks,
			    (void **)log_entries, log_entry_nums,
			    &log_entry_idx);
	...
	winefs_copy_to_edge_blk(sb, pi, over_sblk, start_blk, offset, false,
				start_buf);
	winefs_copy_to_edge_blk(sb, pi, over_eblk, end_blk, eblk_offset, true,
				end_buf);
	...
	if (num_inplace_blks > 0) {
		trans = winefs_new_transaction(sb, max_logentries,
					       winefs_get_cpuid(sb));
		if (IS_ERR(trans)) {
			ret = PTR_ERR(trans);
			goto out;
		...
		for (idx = 0; idx < log_entry_idx; idx++) {
			winefs_add_logentry(sb, trans, (void *)log_entries[idx],
					    (size_t)log_entry_nums[idx] << 3,
					    LE_DATA);

			for (idx2 = 0; idx2 < log_entry_nums[idx]; idx2++) {
				block_val = *(log_entries[idx] + (idx2));
				if (block_val != 0) {
					free_blk_list[free_blk_list_idx] =
						block_val;
					*(log_entries[idx] + (idx2)) =
						inplace_blk_list
							[free_blk_list_idx];
					free_blk_list_idx++;
				}
		...
		written =
			__winefs_xip_file_write(mapping, buf, count, pos, ppos);
		if (written < 0 || written != count)
			winefs_dbg_verbose(
				"write incomplete/failed: written %ld len %ld"
				" pos %llx start_blk %lx num_blocks %lx\n",
			      ...
			      size_t len, loff_t *ppos)
{
	...
	if (sbi->num_numa_nodes > 1 &&
	    pi->numa_node != winefs_get_numa_node(sb, cpu)) {
		proc_numa = &(sbi->process_numa[current->tgid %
						sbi->num_parallel_procs]);
	...
	if (!(strong_guarantees && pos < i_size_read(inode))) {
		...
		same_block = (((count + offset - 1) >>
			       winefs_inode_blk_shift(pi)) == 0) ?
				     1 :
				     0;
		if (block && same_block) {
			WINEFS_START_TIMING(xip_write_fast_t,
					    xip_write_fast_time);
			ret = winefs_file_write_fast(sb, inode, pi, buf, count,
						     pos, ppos, block);
			WINEFS_END_TIMING(xip_write_fast_t,
					  xip_write_fast_time);
			goto out;
		}
	}
	...
	if (strong_guarantees && pos < i_size_read(inode)) {
		winefs_copy_from_edge_blk(sb, pi, over_sblk, start_blk, offset,
					  false, &start_buf);
		winefs_copy_from_edge_blk(sb, pi, over_eblk, end_blk,
					  eblk_offset, true, &end_buf);
	}

	if (pos < i_size_read(inode)) {
		free_blk_list = (__le64 *)kmalloc(num_blocks * sizeof(__le64),
						  GFP_KERNEL);
		num_free_blks = 0;
	}
	...
	winefs_alloc_blocks(trans, inode, start_blk, num_blocks, false, ANY_CPU,
			    1, free_blk_list, &num_free_blks, NULL, NULL, NULL);
	...
	if (strong_guarantees && pos < i_size_read(inode)) {
		winefs_copy_to_edge_blk(sb, pi, over_sblk, start_blk, offset,
					false, start_buf);
		winefs_copy_to_edge_blk(sb, pi, over_eblk, end_blk, eblk_offset,
					true, end_buf);
	}
	...
	if (free_blk_list != NULL && num_free_blks != 0) {
		truncate_strong_guarantees(sb, free_blk_list, num_free_blks,
					   pi->i_blk_type);
		kfree(free_blk_list);
		free_blk_list = NULL;
		num_free_blks = 0;
...
static int winefs_find_and_alloc_blocks(struct inode *inode, sector_t iblock,
					unsigned long max_blocks, u64 *bno,
					int create)
{
	int err = -EIO;
	u64 block;
	...
	if (create == 0) {
		...
		blocks_found = winefs_find_data_blocks_read(inode, iblock,
							    &block, max_blocks);
		WINEFS_END_TIMING(read_winefs_find_data_blocks_t,
				  read_winefs_find_data_blocks_time);
	} else {
		blocks_found = winefs_find_data_blocks(inode, iblock, &block,
						       max_blocks);
	}
	...
	if (blocks_found == 0) {
		...
		if (trans) {
			err = winefs_alloc_blocks_weak(trans, inode, iblock,
						       max_blocks, true,
						       ANY_CPU, 0);
		       ...
		       bool taking_lock)
{
	...
	winefs_dbg_verbose(
		"%s: calling find_and_alloc_blocks. first_block = %lu "
		"max_blocks = %lu. length = %lld\n",
		__func__, first_block, max_blocks, length);

	ret = winefs_find_and_alloc_blocks(inode, first_block, max_blocks, &bno,
					   flags & IOMAP_WRITE);

	if (ret < 0) {
		winefs_dbg("%s: winefs_dax_get_blocks failed %d", __func__,
			   ret);
	...
	if (sbi->num_numa_nodes == 2) {
		...
		if (byte_offset_in_dax >= sbi->initsize) {
			...
			iomap->addr =
				byte_offset_in_dax; //(sector_t)byte_offset_in_dax >> 9;
		}
	}
	...
	winefs_dbg_verbose(
		"%s: iomap->flags %d, iomap->offset %lld, iomap->addr %lu, "
		"iomap->length %llu\n",
		__func__, iomap->flags, iomap->offset, iomap->addr,
		iomap->length);
				     ...
				     u64 *result)
{
	...
	rc = winefs_find_and_alloc_blocks(inode, (sector_t)pgoff, max_blocks,
					  result, create);
	return rc;
}

int winefs_get_xip_mem(struct address_space *mapping, pgoff_t pgoff,
		       unsigned long max_blocks, int create, void **kmem,
		       unsigned long *pfn)
{
	int rc;
	u64 block = 0;
	...
	if (create == 0) {
		WINEFS_START_TIMING(read__winefs_get_block_t,
				    read__winefs_get_block_time);
	}
	...
	if (rc <= 0) {
		winefs_dbg1(
			...
			WINEFS_SB(inode->i_sb)->phys_addr, block, pgoff, create,
			*pfn);
		return rc;
	}
	if (create == 0) {
		WINEFS_END_TIMING(read__winefs_get_block_t,
				  read__winefs_get_block_time);
	}
	...
	*pfn = winefs_get_pfn(inode->i_sb, block);
	...
	winefs_dbg_mmapvv("[%s:%d] sb->physaddr(0x%llx), block(0x%llx),"
			  " pgoff(0x%lx), flag(0x%x), PFN(0x%lx)\n",
			  __func__, __LINE__, WINEFS_SB(inode->i_sb)->phys_addr,
					     ...
					     unsigned int order)
{
	...
	winefs_dbg_verbose("%s: inode %lu, pgoff %lu, order %d\n", __func__,
			   inode->i_ino, vmf->pgoff, order);

	if (vmf->flags & FAULT_FLAG_WRITE)
		file_update_time(vmf->vma->vm_file);

	ret = dax_iomap_fault(vmf, order, &pfn, &error, &winefs_iomap_ops_lock);
...
static vm_fault_t winefs_dax_pfn_mkwrite(struct vm_fault *vmf)
{
	...
	return winefs_xip_huge_file_fault(vmf, 0);
}
...
int winefs_insert_write_vma(struct vm_area_struct *vma)
{
	...
	if (sbi->num_numa_nodes > 1 &&
	    pi->numa_node != sbi->cpu_numa_node[cpu]) {
		proc_numa = &(sbi->process_numa[current->tgid %
						sbi->num_parallel_procs]);
		if (proc_numa->tgid == current->tgid)
			proc_numa->numa_node = pi->numa_node;
...
static void winefs_vma_close(struct vm_area_struct *vma)
{
	winefs_dbg_verbose(
		...
		__func__, __LINE__, vma->vm_start, vma->vm_end, vma->vm_flags,
