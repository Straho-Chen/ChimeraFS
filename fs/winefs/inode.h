...
static inline u64 winefs_get_addr_off(struct winefs_sb_info *sbi, void *addr)
{
...
winefs_get_inode_table_log(struct super_block *sb, int cpu)
{
	...
	return (struct inode_table *)((char *)winefs_get_block(
					      sb, WINEFS_DEF_BLOCK_SIZE_4K *
							  table_start) +
				      cpu * CACHELINE_SIZE);
}
						    ...
						    u64 ino)
{
	...
	for (i = 0; i < superpage_count; i++) {
		...
		curr_addr +=
			winefs_blk_type_to_size[winefs_inode_table->i_blk_type] -
			8;
		curr = *(u64 *)(curr_addr);

		if (curr == 0) {
			allocated = winefs_new_blocks(sb, &blocknr, 1,
						      WINEFS_BLOCK_TYPE_2M, 1,
						      cpuid);
...
static inline struct winefs_inode_truncate_item *
winefs_get_truncate_item(struct super_block *sb, u64 ino)
{
...
static inline struct winefs_inode_truncate_item *
winefs_get_truncate_list_head(struct super_block *sb)
{
...
extern int
__winefs_alloc_blocks(winefs_transaction_t *trans, struct super_block *sb,
...
extern int winefs_alloc_blocks_weak(winefs_transaction_t *trans,
				    struct inode *inode,
				    unsigned long file_blocknr,
				    unsigned int num, bool zero, int cpu,
				    int write_path);
extern unsigned long winefs_find_data_blocks(struct inode *inode,
					     unsigned long file_blocknr,
					     u64 *bp, unsigned long max_blocks);
...
extern struct inode *winefs_new_inode(struct mnt_idmap *idmap,
...
extern int winefs_write_inode(struct inode *inode,
			      struct writeback_control *wbc);
extern void winefs_dirty_inode(struct inode *inode, int flags);
extern int winefs_notify_change(struct mnt_idmap *idmap, struct dentry *dentry,
				struct iattr *attr);
int winefs_getattr(struct mnt_idmap *idmap, const struct path *path,
		   struct kstat *stat, u32 request_mask, unsigned int flags);
extern void winefs_set_inode_flags(struct inode *inode,
				   struct winefs_inode *pi);
extern void winefs_get_inode_flags(struct inode *inode,
				   struct winefs_inode *pi);
