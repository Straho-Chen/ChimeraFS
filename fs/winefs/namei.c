 ...
 */
static inline void winefs_inc_count(struct inode *inode,
...
static inline void winefs_dec_count(struct inode *inode,
				    struct winefs_inode *pi)
{
	if (inode->i_nlink) {
		drop_nlink(inode);
...
static inline struct winefs_direntry *
winefs_next_entry(struct winefs_direntry *p)
{
				  ...
				  struct winefs_direntry **res_entry)
{
	...
	direntry = winefs_find_dentry(sb, NULL, dir, entry->name, entry->len);
	if (direntry == NULL) {
		return 0;
	}
 ...
 * with d_instantiate().
 */
static int winefs_create(struct mnt_idmap *idmap, struct inode *dir,
			 struct dentry *dentry, umode_t mode, bool excl)
{
	...
	trans = winefs_new_transaction(
		sb, MAX_INODE_LENTRIES * 2 + MAX_DIRENTRY_LENTRIES,
		winefs_get_cpuid(sb));
	if (IS_ERR(trans)) {
		err = PTR_ERR(trans);
		goto out;
	...
	inode = winefs_new_inode(idmap, trans, dir, mode, &dentry->d_name);
...
static int winefs_mknod(struct mnt_idmap *idmap, struct inode *dir,
...
static int winefs_symlink(struct mnt_idmap *idmap, struct inode *dir,
			  struct dentry *dentry, const char *symname)
{
	...
	inode = winefs_new_inode(idmap, trans, dir, S_IFLNK | S_IRWXUGO,
				 &dentry->d_name);
	err = PTR_ERR(inode);
	if (IS_ERR(inode)) {
		winefs_abort_transaction(sb, trans);
...
static int winefs_mkdir(struct mnt_idmap *idmap, struct inode *dir,
			struct dentry *dentry, umode_t mode)
{
	...
	inode = winefs_new_inode(idmap, trans, dir, S_IFDIR | mode,
				 &dentry->d_name);
	err = PTR_ERR(inode);
	if (IS_ERR(inode)) {
		winefs_abort_transaction(sb, trans);
	 ...
	 */
	err = winefs_alloc_blocks_weak(NULL, inode, 0, 1, false, ANY_CPU, 0);
	if (err)
	...
	winefs_flush_buffer(
		blk_base, WINEFS_DIR_REC_LEN(1) + WINEFS_DIR_REC_LEN(2), true);
...
static int winefs_rename(struct mnt_idmap *idmap, struct inode *old_dir,
			 ...
			 struct dentry *new_dentry, unsigned int flags)
{
	...
	trans = winefs_new_transaction(
		sb, MAX_INODE_LENTRIES * 4 + MAX_DIRENTRY_LENTRIES * 2,
		winefs_get_cpuid(sb));
	if (IS_ERR(trans)) {
	...
	if (!new_de) {
		...
		winefs_add_logentry(sb, trans, &new_de->ino,
