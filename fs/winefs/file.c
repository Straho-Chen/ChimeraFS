			     ...
			     loff_t len)
{
	...
	trans = winefs_new_transaction(
		sb, MAX_INODE_LENTRIES + MAX_METABLOCK_LENTRIES,
		winefs_get_cpuid(sb));
	if (IS_ERR(trans)) {
		ret = PTR_ERR(trans);
		goto out;
	...
	ret = winefs_alloc_blocks_weak(trans, inode, blocknr, num_blocks, true,
				       ANY_CPU, 0);
...
int winefs_fsync(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	do {
		...
		if (block) {
			...
			winefs_flush_buffer(xip_mem + offset, nr_flush_bytes,
					    0);
		} else {
