...
#define IS_BLOCK_2MB_ALIGNED(block) (!(block & PAGES_PER_2MB_MASK))
					   ...
					   unsigned long ino_high)
{
	...
	ret = winefs_find_free_slot(tree, internal_low, internal_high, &prev,
				    &next);
	if (ret) {
...
static void winefs_destroy_blocknode_tree(struct super_block *sb, int cpu)
{
	...
	winefs_destroy_range_node_tree(
		sb, &free_list->huge_aligned_block_free_tree);
}
...
static int winefs_init_blockmap_from_inode(struct super_block *sb)
{
	...
	winefs_dbg(
		"total_blocknodes = %lu, num_blocks = %lu, num_free_blocks = %lu, blocknode mapping file size = %llu\n",
		total_blocknodes, sbi->num_blocks, sbi->num_free_blocks,
		pi->i_size);

	for (block_ctr = 0; block_ctr < pi->i_size / sbi->blocksize;
	     block_ctr++) {
		...
		for (node_ctr = 0; node_ctr < RANGENODE_PER_PAGE; node_ctr++) {
			...
			entry = (struct winefs_range_node_lowhigh *)
				winefs_get_block(sb, curr_p);
			...
			if (blknode->range_low == 0 &&
			    blknode->range_high == 0) {
			...
			if (blknode->range_low % 512 == 0 &&
				...
				if (free_list->num_blocknode_unaligned == 1)
					free_list->first_node_unaligned =
						blknode;
			}
			if (ret) {
				winefs_err(sb, "%s failed\n", __func__);
...
out:
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		if (temp) {
			curr = container_of(temp, struct winefs_range_node,
					    node);
...
static int winefs_init_inode_list_from_inode(struct super_block *sb)
{
	...
	winefs_dbg("%s: total inode nodes = %lu, pi->i_size = %Lu\n", __func__,
		   total_inode_nodes, pi->i_size);
	for (block_ctr = 0; block_ctr < pi->i_size / sbi->blocksize;
	     block_ctr++) {
		__winefs_find_data_blocks(sb, pi, block_ctr, &curr_p, 1);

		for (inode_node_ctr = 0; inode_node_ctr < RANGENODE_PER_PAGE;
		     inode_node_ctr++) {
			...
			if (range_node->range_low == 0 &&
			    range_node->range_high == 0) {
				winefs_free_inode_node(sb, range_node);
				goto next;
			}
			...
			sbi->s_inodes_used_count += range_node->range_high -
						    range_node->range_low + 1;
			num_inode_node++;
						 ...
						 unsigned long num_blocks)
{
	...
	errval = __winefs_alloc_blocks_wrapper(trans, sb, pi, 0, num_blocks,
					       false, 0, 0);
	return errval;
}
...
static u64 winefs_save_range_nodes(struct super_block *sb,
				   struct winefs_inode *pi,
				   struct rb_root *tree, u64 blocknode_ctr,
				   unsigned long cpuid)
{
	struct winefs_range_node *curr;
	struct rb_node *temp;
	...
	while (temp) {
		...
		winefs_append_range_node_entry(sb, curr, (void *)p, cpuid);
		p += (unsigned long)size;
		temp = rb_next(temp);
					    ...
					    u64 blocknode_ctr)
{
	...
	free_list = winefs_get_free_list(sb, cpu);
	blocknode_ctr = winefs_save_range_nodes(
		sb, pi, &free_list->unaligned_block_free_tree, blocknode_ctr,
		0);
	blocknode_ctr = winefs_save_range_nodes(
		sb, pi, &free_list->huge_aligned_block_free_tree, blocknode_ctr,
		0);
	return blocknode_ctr;
}
...
void winefs_save_blocknode_mappings(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		free_list = winefs_get_free_list(sb, i);
		num_blocknode += free_list->num_blocknode_unaligned +
				 free_list->num_blocknode_huge_aligned;
	}
	...
	winefs_dbg(
		"%s: num blocknodes to save = %lu. num pages = %lu, num_blocknode_allocated = %lu\n",
	...
	*/
	...
	trans = winefs_new_transaction(
		sb,
		MAX_INODE_LENTRIES + MAX_METABLOCK_LENTRIES + MAX_SB_LENTRIES,
		0);
	if (IS_ERR(trans))
		return;
	...
	errval =
		winefs_allocate_datablock_block_inode(trans, sb, pi, num_pages);
	...
	for (i = 0; i < sbi->cpus; i++) {
		blocknode_ctr = winefs_save_free_list_blocknodes(sb, i, pi,
								 blocknode_ctr);
	}
	...
	winefs_memunlock_range(sb, &super->s_wtime,
			       WINEFS_FAST_MOUNT_FIELD_SIZE);
...
void winefs_save_inode_list(struct super_block *sb)
{
	...
	*/
	/* 2 log entry for inode, 2 lentry for super-block */
	trans = winefs_new_transaction(sb, MAX_INODE_LENTRIES + MAX_SB_LENTRIES,
				       0);
	if (IS_ERR(trans))
		return;
	...
	for (i = 0; i < sbi->cpus; i++) {
		inode_map = &sbi->inode_maps[i];
		blocknode_ctr = winefs_save_range_nodes(
			sb, pi, &inode_map->inode_inuse_tree, blocknode_ctr, i);
	}
...
static int winefs_insert_blocknode_map(struct super_block *sb, int cpuid,
					...
					unsigned long scale)
{
	...
	*/
	...
	while (1) {
		...
		if (winefs_insert_blocknode_map(sb, cpuid, low << scale,
						(next << scale) - 1)) {
...
static void winefs_update_4K_map(struct super_block *sb, struct scan_bitmap *bm,
					 ...
					 u8 btype)
{
	...
	for (i = 0; i < (1 << META_BLK_SHIFT); i++) {
		if (node[i] == 0)
			continue;
		winefs_inode_crawl_recursive(sb, bm, le64_to_cpu(node[i]),
					     height - 1, btype);
	}
}
		...
		*ino_low = *ino_high = pi->winefs_ino;
		...
		if (pi->winefs_ino == *ino_high + sbi->cpus) {
			*ino_high = pi->winefs_ino;
			...
			/* A new start */
			winefs_failure_insert_inodetree(sb, *ino_low,
							*ino_high);
			*ino_low = *ino_high = pi->winefs_ino;
		}
	}
...
static int failure_thread_func(void *data)
{
	...
	for (count = 0; count < ring->num; count++) {
		...
		for (i = 0; i < num_inodes_per_page; i++) {
			...
			winefs_failure_update_inodetree(sb, pi, &ino_low,
							&ino_high);
...
int winefs_recovery(struct super_block *sb, size_t size, size_t size_2)
{
	...
	journal_data_start =
		(INODE_TABLE0_START + INODE_TABLE_NUM_BLOCKS) * sbi->blocksize;
	winefs_init_blockmap(sb, journal_data_start + (sbi->cpus * sbi->jsize),
			     1);
