				      ...
				      struct winefs_sb_info *sbi)
{
	...
	size_2 = dax_direct_access(sbi->s_dax_dev,
				   ...
				   LONG_MAX / PAGE_SIZE, DAX_ACCESS,
				   &virt_addr_2, &__pfn_t_2) *
		 PAGE_SIZE;
	if (size_2 <= 0) {
	...
	winefs_info(
		"%s: sbi->virt_addr = 0x%016llx, sbi->phys_addr = 0x%016llx "
		"sbi->virt_addr_2 = 0x%016llx, sbi->phys_addr_2 = 0x%016llx\n",
		__func__, sbi->virt_addr, sbi->phys_addr, sbi->virt_addr_2,
		sbi->phys_addr_2);
				...
				bool remount)
{
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_hugemmap:
			...
			winefs_info(
				"WINEFS: Enabling huge mappings for mmap\n");
			break;
...
static bool winefs_check_size(struct super_block *sb, unsigned long size)
{
	...
	if (sbi->num_inodes > 0)
		num_blocks = (sbi->num_inodes >>
			      (sb->s_blocksize_bits - WINEFS_INODE_BITS)) +
			     1;
	else
		num_blocks = 1;
...
static struct winefs_inode *
winefs_init(struct super_block *sb, unsigned long size, unsigned long size_2)
{
	...
	if (!winefs_check_size(sb, size + size_2)) {
		winefs_dbg(
			"Specified WINEFS size too small 0x%lx. Either increase"
	...
	for (idx = 0; idx < sbi->cpus; idx++) {
		...
		journal_meta_start[idx] =
			(journal_meta_start[idx] + CACHELINE_SIZE - 1) &
			~(CACHELINE_SIZE - 1);
		winefs_dbg_verbose("journal_meta_start[%d] = 0x%Lx\n", idx,
				   journal_meta_start[idx]);
		journal_start =
			journal_meta_start[idx] + sizeof(winefs_journal_t);
	}
	...
	if ((inode_table_start + sizeof(struct winefs_inode)) >
	    WINEFS_SB_SIZE) {
		winefs_dbg(
			"WINEFS super block defined too small. defined 0x%x, "
			"required 0x%llx\n",
			WINEFS_SB_SIZE,
	...
	winefs_dbg_verbose("journal meta start %llx data start 0x%llx, "
			   ...
			   journal_meta_start[0], journal_data_start,
	...
	winefs_init_blockmap(sb, journal_data_start + (sbi->jsize * sbi->cpus),
			     0);
	...
	winefs_flush_buffer((char *)super + WINEFS_SB_SIZE, sizeof(*super),
			    false);
	...
	de = (struct winefs_direntry *)winefs_get_block(
		sb, winefs_get_block_off(sb, blocknr, WINEFS_BLOCK_TYPE_4K));
...
static void winefs_root_check(struct super_block *sb,
...
static int winefs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!sbi->inode_maps) {
		retval = -ENOMEM;
		winefs_dbg("%s: Allocating inode maps failed.", __func__);
		goto out;
	}
	...
	for (i = 0; i < sbi->num_numa_nodes; i++) {
		sbi->numa_cpus[i].cpus =
			kcalloc(sbi->cpus, sizeof(int), GFP_KERNEL);
	...
	if (sbi->num_numa_nodes == 1) {
		...
		if (sbi->cpus == 96) {
			for (i = 0; i < sbi->cpus; i++) {
				if (i < 24 || (i >= 48 && i < 72)) {
					sbi->numa_cpus[0]
						.cpus[sbi->numa_cpus[0]
							      .num_cpus] = i;
					sbi->numa_cpus[0].num_cpus++;
					sbi->cpu_numa_node[i] = 0;
					cpumask_set_cpu(
						i, &sbi->numa_cpus[0].cpumask);
				} else {
					sbi->numa_cpus[1]
						.cpus[sbi->numa_cpus[1]
							      .num_cpus] = i;
					sbi->numa_cpus[1].num_cpus++;
					sbi->cpu_numa_node[i] = 1;
					cpumask_set_cpu(
						i, &sbi->numa_cpus[1].cpumask);
				}
			}
		} else if (sbi->cpus == 32) {
			for (i = 0; i < sbi->cpus; i++) {
	...
	sbi->process_numa = kcalloc(sbi->num_parallel_procs,
				    sizeof(struct process_numa), GFP_KERNEL);

	for (i = 0; i < sbi->num_parallel_procs; i++)
		sbi->process_numa[i].numa_node = -1;

	sbi->block_start =
		kcalloc(sbi->num_numa_nodes, sizeof(unsigned long), GFP_KERNEL);
	sbi->block_end =
		kcalloc(sbi->num_numa_nodes, sizeof(unsigned long), GFP_KERNEL);
	...
	winefs_dbg_verbose(
		"checking physical address 0x%016llx for pmfs image\n",
		(u64)sbi->phys_addr);
...
void winefs_free_inode_node(struct super_block *sb,
...
void winefs_free_vma_item(struct super_block *sb, struct vma_item *item)
{
	winefs_free_range_node((struct winefs_range_node *)item);
}

void winefs_free_blocknode(struct super_block *sb,
			   struct winefs_range_node *node)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	sbi->num_blocknode_allocated--;
...
{
	return (winefs_transaction_t *)kmem_cache_alloc(
		winefs_transaction_cachep, GFP_NOFS);
}
...
struct winefs_range_node *winefs_alloc_range_node_atomic(struct super_block *sb)
{
	...
	p = (struct winefs_range_node *)kmem_cache_zalloc(
		winefs_range_node_cachep, GFP_ATOMIC);
	return p;
}
...
struct winefs_range_node *winefs_alloc_range_node(struct super_block *sb)
{
	...
	p = (struct winefs_range_node *)kmem_cache_zalloc(
		winefs_range_node_cachep, GFP_NOFS);
	return p;
}
...
{
	winefs_range_node_cachep = kmem_cache_create(
		"winefs_range_node_cache", sizeof(struct winefs_range_node), 0,
...
{
	winefs_inode_cachep = kmem_cache_create(
		"winefs_inode_cache", sizeof(struct winefs_inode_info), 0,
...
{
	winefs_transaction_cachep = kmem_cache_create(
		"winefs_journal_transaction", sizeof(winefs_transaction_t), 0,
		(SLAB_RECLAIM_ACCOUNT | SLAB_MEM_SPREAD), NULL);
...
static struct dentry *winefs_mount(struct file_system_type *fs_type, int flags,
...
static struct inode *winefs_nfs_get_inode(struct super_block *sb, u64 ino,
					  u32 generation)
{
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct inode *inode;
...
module_init(init_winefs_fs) module_exit(exit_winefs_fs)
