...
uint32_t winefs_blk_type_to_size[WINEFS_BLOCK_TYPE_MAX] = { 0x1000, 0x200000,
							    0x40000000 };
 ...
 */
static int winefs_new_data_blocks(struct super_block *sb,
				  ...
				  int zero, int cpu, int write_path)
{
	...
	allocated =
		winefs_new_blocks(sb, blocknr, num, pi->i_blk_type, zero, cpu);
...
unsigned long winefs_find_data_blocks_read(struct inode *inode,
					   unsigned long file_blocknr, u64 *bp,
					   unsigned long max_blocks)
{
	...
	WINEFS_START_TIMING(__winefs_find_data_blocks_t,
			    __winefs_find_data_blocks_time);
	...
	if (blocknr >= (1UL << (pi->height * meta_bits))) {
		*bp = 0;
		WINEFS_END_TIMING(__winefs_find_data_blocks_t,
				  __winefs_find_data_blocks_time);
		return 0;
	}

	num_blocks_found =
		__winefs_find_data_blocks(sb, pi, blocknr, bp, max_blocks);
			       ...
			       unsigned long num_blocks, u32 btype)
{
	...
	while (i <= last_index) {
		for (j = i; j <= last_index; j++) {
			...
			blocknr = winefs_get_blocknr(sb, le64_to_cpu(node[j]),
						     btype);

			if (blocknr != (prev_blocknr + 1) &&
			    prev_blocknr != 0) {
				blocknr = winefs_get_blocknr(
					sb, le64_to_cpu(node[i]), btype);
	...
	if (height == 1) {
			...
			for (j = i; j <= last_index; j++) {
				...
				if (blocknr != (prev_blocknr + 1) &&
					...
					blocknr = winefs_get_blocknr(
						...
						btype);
					winefs_free_blocks(sb, blocknr, (j - i),
							   btype);
					freed += (j - i);
					i = j;
					prev_blocknr = 0;
		...
		for (i = first_index; i <= last_index; i++) {
			...
			if (mpty) {
				...
				winefs_free_blocks(sb, blocknr, 1,
						   WINEFS_BLOCK_TYPE_4K);
			} else {
				if (i == first_index)
					start++;
...
unsigned int winefs_free_inode_subtree(struct super_block *sb, __le64 root,
				       u32 height, u32 btype,
				       unsigned long last_blocknr)
{
	unsigned long first_blocknr;
	...
	if (height == 0) {
		first_blocknr =
			winefs_get_blocknr(sb, le64_to_cpu(root), btype);
...
winefs_inode_count_iblocks_recursive(struct super_block *sb, __le64 block,
				     u32 height)
{
	__le64 *node;
	unsigned int i;
...
winefs_sparse_last_blocknr(unsigned int height, unsigned long last_blocknr)
{
				     ...
				     loff_t end)
{
	...
	if (pi->i_flags & cpu_to_le32(WINEFS_EOFBLOCKS_FL)) {
		...
		last_blocknr =
			winefs_sparse_last_blocknr(pi->height, last_blocknr);
	}
	...
	if (pi->height == 0) {
		...
		freed = recursive_truncate_blocks(sb, root, pi->height,
						  ...
						  last_blocknr, &mpty);
		if (mpty) {
			first_blocknr = winefs_get_blocknr(
				sb, le64_to_cpu(root), WINEFS_BLOCK_TYPE_4K);
			winefs_free_blocks(sb, first_blocknr, 1,
					   WINEFS_BLOCK_TYPE_4K);
			root = 0;
		}
	}
					...
					struct winefs_inode *pi, u32 new_height)
{
	...
	while (height < new_height) {
		...
		blocknr =
			winefs_get_block_off(sb, blocknr, WINEFS_BLOCK_TYPE_4K);
 ...
 */
static int
recursive_alloc_blocks(winefs_transaction_t *trans, struct super_block *sb,
		       struct winefs_inode *pi, __le64 block, u32 height,
		       unsigned long first_blocknr, unsigned long last_blocknr,
		       bool new_node, bool zero, int cpu, int write_path,
		       ...
		       int *log_entry_idx)
{
	...
	int allocated, freed;
	__le64 node_val;
	bool strong_guarantees = WINEFS_SB(sb)->s_mount_opt &
				 WINEFS_MOUNT_STRICT;
	...
	while (i <= last_index) {
		if (height == 1) {
			if (node[i] == 0 || (free_blk_list != NULL)) {
				...
				allocated = winefs_new_data_blocks(
					sb, pi, &blocknr, num_blocks, zero, cpu,
					write_path);
				if (allocated <= 0) {
					winefs_dbg(
						"%s: alloc %d blocks failed!, %d\n",
						__func__, num_blocks,
						allocated);
					winefs_memunlock_inode(sb, pi);
				...
				if (new_node == 0 && journal_saved == 0) {
					...
					if (log_entries != NULL) {
						log_entries[*log_entry_idx] =
							&node[i];
						log_entry_nums[*log_entry_idx] =
							(last_index - i + 1);
						(*log_entry_idx) += 1;
					}
				...
				for (j = i; j < i + allocated; j++) {
					node_val = cpu_to_le64(
						winefs_get_block_off(
							sb, blocknr,
							pi->i_blk_type));
			...
			if (node[i] == 0) {
				...
				winefs_memunlock_block(sb, node);
				node[i] = cpu_to_le64(winefs_get_block_off(
					sb, blocknr, WINEFS_BLOCK_TYPE_4K));
				winefs_memlock_block(sb, node);
				new_node = 1;
			}
			...
			errval = recursive_alloc_blocks(
				trans, sb, pi, node[i], height - 1, first_blk,
				last_blk, new_node, zero, cpu, write_path,
				free_blk_list, num_free_blks, log_entries,
				log_entry_nums, log_entry_idx);
			if (errval < 0)
				goto fail;
			i++;
...
int __winefs_alloc_blocks(winefs_transaction_t *trans, struct super_block *sb,
			  struct winefs_inode *pi, unsigned long file_blocknr,
			  ...
			  int *log_entry_idx)
{
	...
	if (!pi->root) {
		if (height == 0) {
			__le64 root;
			errval = winefs_new_data_blocks(sb, pi, &blocknr, 1,
							zero, cpu, write_path);
			if (errval < 0) {
			...
			root = cpu_to_le64(winefs_get_block_off(
			...
			errval = recursive_alloc_blocks(
				trans, sb, pi, pi->root, pi->height,
				first_blocknr, last_blocknr, 1, zero, cpu,
				write_path, free_blk_list, num_free_blks,
				log_entries, log_entry_nums, log_entry_idx);
			if (errval < 0)
				goto fail;
		}
	} else {
...
int __winefs_alloc_blocks_wrapper(winefs_transaction_t *trans,
				  struct super_block *sb,
				  struct winefs_inode *pi,
				  unsigned long file_blocknr, unsigned int num,
				  bool zero, int cpu, int write_path)
{
	return __winefs_alloc_blocks(trans, sb, pi, file_blocknr, num, zero,
				     cpu, write_path, NULL, NULL, NULL, NULL,
				     NULL);
}
			       ...
			       __le64 *log_entry_nums, int *log_entry_idx)
{
	...
	errval = __winefs_alloc_blocks(trans, sb, pi, file_blocknr, num, zero,
				       cpu, write_path, free_blk_list,
				       num_free_blks, log_entries,
				       log_entry_nums, log_entry_idx);
 ...
 * block number.
 */
inline int winefs_alloc_blocks_weak(winefs_transaction_t *trans,
				    struct inode *inode,
				    ...
				    int write_path)
{
	...
	errval = __winefs_alloc_blocks_wrapper(trans, sb, pi, file_blocknr, num,
					       zero, cpu, write_path);
				   ...
				   struct winefs_range_node **ret_node)
{
	...
	return winefs_find_range_node(tree, internal_ino, NODE_INODE, ret_node);
}
...
static int winefs_rebuild_inode(struct super_block *sb, struct inode *inode,
				struct winefs_inode *pi)
{
	struct winefs_inode_info *si = WINEFS_I(inode);
	struct winefs_inode_info_header *sih = &si->header;
...
void winefs_evict_inode(struct inode *inode)
{
	...
	if (!inode->i_nlink && !is_bad_inode(inode)) {
		...
		winefs_free_inode_subtree(sb, root, height, btype,
					  last_blocknr);
		inode->i_mtime = inode->__i_ctime = current_time(inode);
		inode->i_size = 0;
	}
...
struct inode *winefs_new_inode(struct mnt_idmap *idmap,
			       winefs_transaction_t *trans, struct inode *dir,
			       umode_t mode, const struct qstr *qstr)
{
	...
	pi = winefs_get_inode(sb, ino);
	winefs_dbg_verbose("%s: ino = %lu, winefs_inode = 0x%p", __func__, ino,
			   pi);
...
void winefs_truncate_del(struct inode *inode)
{
	...
	struct super_block *sb = inode->i_sb;
	struct winefs_sb_info *sbi = WINEFS_SB(sb);
	struct winefs_inode_truncate_item *head =
		winefs_get_truncate_list_head(sb);
	...
	if (prev == &sbi->s_truncate) {
		...
		struct inode *i_prv =
			&list_entry(prev, struct winefs_inode_info, i_truncated)
				 ->vfs_inode;
...
int winefs_getattr(struct mnt_idmap *idmap, const struct path *path,
...
static int winefs_update_single_field(struct super_block *sb,
				      struct inode *inode,
				      struct winefs_inode *pi,
...
int winefs_notify_change(struct mnt_idmap *idmap, struct dentry *dentry,
			 struct iattr *attr)
{
