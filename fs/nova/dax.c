			   ...
			   unsigned long blocknr, bool inplace)
{
	...
	if (unlikely(left != 0)) {
		nova_err(
			sb,
			"%s: not all data is copied from user! expect to copy %zu bytes, actually copied %zu bytes\n",
				   ...
				   size_t len, loff_t *ppos)
{
	...
	while (num_blocks > 0) {
		...
		while (remain > 0) {
			...
			page_offset = ((unsigned long)kmem_iter) &
				      (to_copy - 1);
...
#if NOVA_MEMCPY_NT
			ret = memcpy_to_pmem_nocache(kmem_iter, buf_iter,
						     to_copy);
#else
			ret = memcpy_to_pmem_flush(kmem_iter, buf_iter,
						   to_copy);
#endif
