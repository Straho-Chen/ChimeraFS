...
static inline int nova_update_inode_checksum(struct nova_inode *pi) {
  ...
  crc =
      nova_crc32c(~0, (__u8 *)pi, (sizeof(struct nova_inode) - sizeof(__le32)));
...
static inline struct inode_table *nova_get_inode_table(struct super_block *sb,
  ...
  return (struct inode_table *)((char *)nova_get_block(
                                    sb, NOVA_DEF_BLOCK_SIZE_4K * table_start) +
                                cpu * CACHELINE_SIZE);
}
...
static inline int
nova_update_range_node_checksum(struct nova_range_node *node) {
  if (dram_struct_csum)
...
static inline bool nova_range_node_checksum_ok(struct nova_range_node *node) {
  ...
  if (!ret) {
    nova_dbg("%s: checksum failure, vma %p, range low %lu, range high %lu, "
             "csum 0x%x\n",
...
static inline unsigned long get_nvmm(struct super_block *sb,
  ...
  if (entry->pgoff > pgoff ||
      (unsigned long)entry->pgoff + (unsigned long)entry->num_pages <= pgoff) {
    struct nova_sb_info *sbi = NOVA_SB(sb);
    u64 curr;

    curr = nova_get_addr_off(sbi, entry);
    nova_dbg("Entry ERROR: inode %lu, curr 0x%llx, pgoff %lu, entry pgoff "
             "%llu, num %u\n",
  ...
  return (unsigned long)(entry->block >> PAGE_SHIFT) + pgoff - entry->pgoff;
}
...
static inline u64 nova_find_nvmm_block(struct super_block *sb,
   ...
   */
  entryc = &entry_copy;
  if (memcpy(entryc, entry, sizeof(struct nova_file_write_entry)) != NULL)
    return 0;
...
static inline u64 next_log_page(struct super_block *sb, u64 curr) {
  ...
  if (memcpy(&next, &curr_page->page_tail.next_page, sizeof(u64)) == NULL)
    return 1;
...
nova_set_next_page_address(struct super_block *sb,
  ...
  nova_flush_buffer(&curr_page->page_tail, sizeof(struct nova_inode_page_tail),
                    0);
  if (fence)
    PERSISTENT_BARRIER();
}
...
static inline void nova_inc_page_invalid_entries(struct super_block *sb,
  ...
  if (curr_page->page_tail.invalid_entries > curr_page->page_tail.num_entries) {
...
static inline void nova_set_alter_page_address(struct super_block *sb, u64 curr,
  ...
  nova_flush_buffer(&alter_page->page_tail, sizeof(struct nova_inode_page_tail),
                    0);
}
...
static inline void *nova_get_data_csum_addr(struct super_block *sb, u64 strp_nr,
  ...
  data_csum_addr =
      (u8 *)nova_get_block(sb, blockoff) + NOVA_DATA_CSUM_LEN * strp_nr;
