  ...
  for (i = 0; i < num_pages; i++) {
    ...
    if (pentry) {
      ...
      nova_dbgv("%s: pgoff %lu, entry 0x%lx, new 0x%lx\n", __func__, curr_pgoff,
                value, new_value);
      radix_tree_replace_slot(&sih->tree, pentry, (void *)new_value);
      radix_tree_tag_set(&mapping->i_pages, curr_pgoff, PAGECACHE_TAG_DIRTY);
    }
  }
...
static int nova_dax_mmap_update_mapping(
  ...
  ret =
      nova_update_dax_mapping(sb, sih, vma, entry_data, start_pgoff, num_pages);
  if (ret) {
    nova_err(sb, "update DAX mapping return %d\n", ret);
    return ret;
  }

  ret = nova_update_entry_pfn(sb, sih, vma, entry_data, start_pgoff, num_pages);
  if (ret)
...
static int nova_get_dax_cow_range(struct super_block *sb,
  ...
  *num_blocks =
      (base > vma_blocks - start_pgoff) ? vma_blocks - start_pgoff : base;
...
int nova_mmap_to_new_blocks(struct vm_area_struct *vma, unsigned long address) {
  ...
  while (start_blk < end_blk) {
    ...
    from_blockoff = nova_get_block_off(sb, from_blocknr, pi->i_blk_type);
    from_kmem = nova_get_block(sb, from_blockoff);

    if (entryc->reassigned == 0)
      avail_blocks = entryc->num_pages - (start_blk - entryc->pgoff);
    else
      avail_blocks = 1;
    ...
    allocated = nova_new_data_blocks(sb, sih, &blocknr, start_blk, avail_blocks,
    ...
    copied = bytes - memcpy_to_pmem_nocache(to_kmem, from_kmem, bytes);
    ...
    nova_init_file_write_entry(sb, sih, &entry_data, epoch_id, entry_pgoff,
                               copy_blocks, blocknr, time, entry_size);
...
out:
  if (ret < 0)
    nova_cleanup_incomplete_write(sb, sih, blocknr, allocated, begin_tail,
                                  update.tail);
