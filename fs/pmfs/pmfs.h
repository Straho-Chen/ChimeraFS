...
#define PMFS_START_META_TIMING(name, start)     \
	{                                       \
...
#define PMFS_END_META_TIMING(name, start)                                      \
	{                                                                      \
		if (measure_meta_timing) {                                     \
			INIT_TIMING(end);                                      \
			mem_fence();                                           \
...
extern struct inode *pmfs_new_inode(struct mnt_idmap *idmap,
...
extern int pmfs_notify_change(struct mnt_idmap *idmap, struct dentry *dentry,
			      struct iattr *iattr);
...
static inline struct pmfs_super_block *
pmfs_get_redund_super(struct super_block *sb)
{
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
					     ...
					     struct pmfs_inode *pi)
{
	...
	/* pi->i_size, pi->i_ctime, and pi->i_mtime need to be atomically updated.  * So use cmpxchg16b here. */
					 ...
					 unsigned long blocknr)
{
	...
	INIT_TIMING(index);
	PMFS_START_TIMING(index_blocks_t, index);
	...
	PMFS_END_TIMING(index_blocks_t, index);
