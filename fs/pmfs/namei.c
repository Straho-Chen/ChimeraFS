...
static inline int pmfs_add_nondir(pmfs_transaction_t *trans, struct inode *dir,
			 ...
			 struct pmfs_direntry **prev_dir)
{
	...
	while ((char *)de < dlimit) {
		...
		if ((char *)de + namelen <= dlimit &&
			...
			if (!pmfs_check_dir_entry("pmfs_inode_by_name", dir, de,
						  blk_base, offset))
				return -1;
			*res_dir = de;
			if (prev_dir)
 ...
 * with d_instantiate().
 */
static int pmfs_create(struct mnt_idmap *idmap, struct inode *dir,
		       struct dentry *dentry, umode_t mode, bool excl)
{
	...
	inode = pmfs_new_inode(idmap, trans, dir, mode, &dentry->d_name);
	if (IS_ERR(inode))
		goto out_err;
...
static int pmfs_mknod(struct mnt_idmap *idmap, struct inode *dir,
...
static int pmfs_symlink(struct mnt_idmap *idmap, struct inode *dir,
			struct dentry *dentry, const char *symname)
{
	...
	inode = pmfs_new_inode(idmap, trans, dir, S_IFLNK | S_IRWXUGO,
			       &dentry->d_name);
	err = PTR_ERR(inode);
	if (IS_ERR(inode)) {
		pmfs_abort_transaction(sb, trans);
...
static int pmfs_mkdir(struct mnt_idmap *idmap, struct inode *dir,
		      struct dentry *dentry, umode_t mode)
{
	...
	inode = pmfs_new_inode(idmap, trans, dir, S_IFDIR | mode,
			       &dentry->d_name);
	err = PTR_ERR(inode);
	if (IS_ERR(inode)) {
		pmfs_abort_transaction(sb, trans);
	...
	pmfs_flush_buffer(blk_base, PMFS_DIR_REC_LEN(1) + PMFS_DIR_REC_LEN(2),
			  true);
...
static int pmfs_empty_dir(struct inode *inode)
{
	...
	while (offset < inode->i_size) {
		if (!blk_base ||
		    (void *)de >= (void *)(blk_base + sb->s_blocksize)) {
			err = 0;
			blk_base = pmfs_get_block(
				sb,
				pmfs_find_data_block(
...
static int pmfs_rename(struct mnt_idmap *idmap, struct inode *old_dir,
