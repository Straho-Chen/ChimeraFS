 ...
 * with d_instantiate().
 */
static int pmfs_create(struct mnt_idmap *idmap,
						...
						struct dentry *dentry, const char *symname)
{
	...
	inode = pmfs_new_inode(idmap, trans, dir, S_IFLNK | S_IRWXUGO, &dentry->d_name);
	err = PTR_ERR(inode);
	if (IS_ERR(inode))
	{
		pmfs_abort_transaction(sb, trans);
...
static int pmfs_mkdir(struct mnt_idmap *idmap,
					  struct inode *dir, struct dentry *dentry, umode_t mode)
{
	...
	inode = pmfs_new_inode(idmap, trans, dir, S_IFDIR | mode, &dentry->d_name);
	err = PTR_ERR(inode);
	if (IS_ERR(inode))
	{
		pmfs_abort_transaction(sb, trans);
