...
static struct pmfs_inode *pmfs_init(struct super_block *sb, unsigned long size)
{
	...
	de = (struct pmfs_direntry *)pmfs_get_block(
...
struct pmfs_blocknode *pmfs_alloc_blocknode(struct super_block *sb)
{
	...
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
	p = (struct pmfs_blocknode *)kmem_cache_alloc(pmfs_blocknode_cachep,
						      GFP_NOFS);
	if (p) {
		sbi->num_blocknode_allocated++;
	}
...
{
	pmfs_blocknode_cachep = kmem_cache_create(
		"pmfs_blocknode_cache", sizeof(struct pmfs_blocknode), 0,
