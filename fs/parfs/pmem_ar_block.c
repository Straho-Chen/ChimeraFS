...
{
	if (cmd == SLFS_PMEM_AR_CMD_CREATE) {
		return pmem_ar_create(arg);
	} else if (cmd == SLFS_PMEM_AR_CMD_DELETE) {
		pmem_ar_delete();
		return 0;
	} else {
...
void put_pmem_ar(void)
{
	...
	for (i = 0; i < pmem_ar_dev.elem_num; i++) {
		if (pmem_ar_dev.dax_dev[i])
			fs_put_dax(pmem_ar_dev.dax_dev[i], &pmem_ar_dev);
	}
}
...
int pmem_ar_init_block(void)
{
	...
	if ((ret = register_blkdev(SLFS_MAJOR, SLFS_DEV_NAME)) < 0) {
		return ret;
	}

	pmem_ar_dev.gd = blk_alloc_disk(SLFS_PMEM_AR_NUM);

	if (pmem_ar_dev.gd == NULL) {
		printk("ERROR: blk_alloc_disk failed!\n");
		goto out_error;
	}

	pmem_ar_dev.gd->major = SLFS_MAJOR;
	pmem_ar_dev.gd->first_minor = 0;
	pmem_ar_dev.gd->fops = &pmem_ar_ops;
	pmem_ar_dev.gd->private_data = &pmem_ar_dev;
	strcpy(pmem_ar_dev.gd->disk_name, SLFS_DEV_INSTANCE_NAME);
...
out_error:
	unregister_blkdev(SLFS_MAJOR, SLFS_DEV_NAME);
	return -ENOMEM;
}
