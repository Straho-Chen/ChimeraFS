...
#define odinfs_init_global_rglock_bravo(l)                    \
	{                                                     \
		l = vzalloc(TABLE_SIZE * sizeof(atomic_t *)); \
	}

#define odinfs_free_global_rglock_bravo(l) \
	{                                  \
		vfree(l);                  \
	}
#endif

extern struct kmem_cache *odinfs_rangelock_cachep;

struct range_lock *range_lock_init(unsigned long size);
void range_lock_free(struct range_lock *lock);

void range_read_lock(struct range_lock *lock, unsigned long start,
		     unsigned long size);
void range_read_unlock(struct range_lock *lock, unsigned long start,
		       unsigned long size);
void range_write_lock(struct range_lock *lock, unsigned long start,
		      unsigned long size);
void range_write_unlock(struct range_lock *lock, unsigned long start,
			unsigned long size);
...
#ifndef RGLOCK_BRAVO
#define odinfs_init_range_lock(l)                                              \
	{                                                                      \
		(l)->sg_table =                                                \
			kmem_cache_alloc(odinfs_rangelock_cachep, GFP_KERNEL); \
...
#define odinfs_init_range_lock(l)                                              \
	{                                                                      \
		(l)->sg_table =                                                \
			vzalloc(SEGMENT_INIT_COUNT * sizeof(struct sg_entry)); \
		(l)->sg_size = SEGMENT_INIT_COUNT;                             \
	}
#else
#define odinfs_init_range_lock(l)                                              \
...
#define get_range_lock(i) get_rangelock_info(ODINFS_I((i)))
...
static inline void odinfs_resize_range_lock(struct range_lock *l, long new_size)
{
	long old_sg_size;
	int need_realloc = 0;
	...
	if (need_realloc) {
		...
		if (old_sg_size == SEGMENT_INIT_COUNT)
			kmem_cache_free(odinfs_rangelock_cachep, l->sg_table);
		else
			vfree(l->sg_table);
#else
		vfree(l->sg_table);
...
read_range_lock(volatile struct range_lock *lock, long start, long end);
...
#define RANGE_READ_LOCK(l, s, e)                                               \
	{                                                                      \
		node = range_read_lock((volatile struct range_lock *)(l), (s), \
				       (e));                                   \
	}
