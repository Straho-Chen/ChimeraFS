...
static inline int
odinfs_rbtree_compare_range_node(struct odinfs_range_node *curr,
...
{
	struct odinfs_range_node *curr = NULL;
	struct rb_node *temp;
	int compVal;
	int ret = 0;
	...
	while (temp) {
		...
		compVal = odinfs_rbtree_compare_range_node(curr, key, type);
			     ...
			     enum node_type type)
{
	...
	while (*temp) {
		curr = container_of(*temp, struct odinfs_range_node, node);
		compVal = odinfs_rbtree_compare_range_node(
		...
		if (compVal == -1) {
			...
			odinfs_dbg("%s: entry %lu - %lu already exists: "
				   "%lu - %lu\n",
...
void odinfs_destroy_range_node_tree(struct rb_root *tree)
{
	...
	while (temp) {
		...
		odinfs_free_range_node(curr);
	}
}

int odinfs_insert_blocktree(struct rb_root *tree,
			    struct odinfs_range_node *new_node)
{
	int ret;

	ret = odinfs_insert_range_node(tree, new_node, NODE_BLOCK);
	if (ret)
...
int odinfs_alloc_block_free_lists(struct super_block *sb)
{
	...
	struct odinfs_free_list *free_list;
	int i, j;

	sbi->free_lists = kcalloc(sbi->cpus * sbi->sockets,
				  sizeof(struct odinfs_free_list), GFP_KERNEL);
	...
	for (i = 0; i < sbi->cpus; i++)
		for (j = 0; j < sbi->sockets; j++) {
			free_list = odinfs_get_free_list(sb, i, j);
...
// manage.
static void odinfs_init_free_list(struct super_block *sb,
				  struct odinfs_free_list *free_list, int cpu,
				  int socket)
{
	struct odinfs_sb_info *sbi = ODINFS_SB(sb);
...
void odinfs_init_blockmap(struct super_block *sb, int recovery)
{
	struct odinfs_sb_info *sbi = ODINFS_SB(sb);
	struct rb_root *tree;
	struct odinfs_range_node *blknode;
	struct odinfs_free_list *free_list;
	int i, j;
	int ret;
	...
	for (i = 0; i < sbi->cpus; i++)
		for (j = 0; j < sbi->sockets; j++) {
			...
			odinfs_init_free_list(sb, free_list, i, j);
			...
			if (recovery == 0) {
				...
				ret = odinfs_insert_blocktree(tree, blknode);
				if (ret) {
					odinfs_err(sb, "%s failed\n", __func__);
					odinfs_free_blocknode(blknode);
					return;
				}
...
static void odinfs_block_to_cpu_socket(struct super_block *sb, int blocknr,
				       int *cpu, int *socket)
{
	int cpu_tmp = 0;
	...
	*socket = odinfs_block_to_socket(sbi, blocknr);
...
int odinfs_find_free_slot(struct rb_root *tree, unsigned long range_low,
			  unsigned long range_high,
			  struct odinfs_range_node **prev,
			  struct odinfs_range_node **next)
{
	struct odinfs_range_node *ret_node = NULL;
	struct rb_node *tmp;
	int ret;

	ret = odinfs_find_range_node(tree, range_low, NODE_BLOCK, &ret_node);
	if (ret) {
		odinfs_dbg("%s ERROR: %lu - %lu already in free list\n",
	...
	if (!ret_node) {
		...
		*prev = ret_node;
		...
		if (tmp) {
			*next = container_of(tmp, struct odinfs_range_node,
					     node);
		} else {
			*next = NULL;
		}
		...
		*next = ret_node;
		...
		if (tmp) {
			*prev = container_of(tmp, struct odinfs_range_node,
					     node);
		} else {
			*prev = NULL;
		}
	} else {
		odinfs_dbg("%s ERROR: %lu - %lu overlaps with existing "
			   "node %lu - %lu\n",
...
static int odinfs_free_blocks(struct super_block *sb, unsigned long blocknr,
			      unsigned long num_blocks)
{
	struct rb_root *tree;
	unsigned long block_low;
	unsigned long block_high;
	struct odinfs_range_node *prev = NULL;
	struct odinfs_range_node *next = NULL;
	...
	if (num_blocks <= 0) {
		odinfs_dbg("%s ERROR: free %lu\n", __func__, num_blocks);
		return -EINVAL;
	}

	odinfs_block_to_cpu_socket(sb, blocknr, &cpuid, &socket);
	...
	free_list = odinfs_get_free_list(sb, cpuid, socket);
	...
	odinfs_dbg_alloc("Free: %lx - %lx to cpu: %d, socket: %d\n", block_low,
	...
	ret = odinfs_find_free_slot(tree, block_low, block_high, &prev, &next);

	if (ret) {
		odinfs_dbg("%s: find free slot fail: %d\n", __func__, ret);
		goto out;
	}
	...
	new_node_used = 1;
	ret = odinfs_insert_blocktree(tree, curr_node);
	if (ret) {
		new_node_used = 0;
		goto out;
	}
...
out:
	...
	if (new_node_used == 0)
		odinfs_free_blocknode(curr_node);
			    ...
			    int num)
{
	...
	if (blocknr == 0) {
		odinfs_dbg("%s: ERROR: %lu, %d\n", __func__, blocknr, num);
		return -EINVAL;
	}

	ret = odinfs_free_blocks(sb, blocknr, num);
...
static int not_enough_blocks(struct odinfs_free_list *free_list,
			     unsigned long num_blocks)
{
	struct odinfs_range_node *first = free_list->first_node;
	struct odinfs_range_node *last = free_list->last_node;
   ...
   */
	if (!first || !last ||
	    free_list->num_free_blocks / free_list->num_blocknode <
		    num_blocks) {
		odinfs_dbg_alloc("%s: num_free_blocks=%ld; num_blocks=%ld; "
...
static int odinfs_get_candidate_free_list(struct super_block *sb, int socket)
{
	struct odinfs_sb_info *sbi = ODINFS_SB(sb);
	struct odinfs_free_list *free_list;
	int cpuid = 0;
	...
	for (i = 0; i < sbi->cpus; i++) {
		free_list = odinfs_get_free_list(sb, i, socket);
...
static long odinfs_alloc_blocks_in_free_list(struct super_block *sb,
					     struct odinfs_free_list *free_list,
					     unsigned long num_blocks,
					     unsigned long *new_blocknr)
{
	struct rb_root *tree;
	struct odinfs_range_node *curr, *next = NULL, *prev = NULL;
	...
	if (!free_list->first_node || free_list->num_free_blocks == 0) {
		odinfs_dbg("%s: Can't alloc. free_list->first_node=0x%p "
...
next:
	...
	if (free_list->num_free_blocks < num_blocks) {
		odinfs_dbg("%s: free list has %lu free blocks, "
	...
	if (found == 1)
		...
		odinfs_dbg("%s: Can't alloc.  found = %d", __func__, found);
		return -ENOSPC;
	}
		      ...
		      unsigned long num_blocks, int zero, int cpu, int socket)
{
	...
	if (num_blocks == 0) {
		odinfs_dbg_verbose("%s: num_blocks == 0", __func__);
		return -EINVAL;
	}

retry:
	free_list = odinfs_get_free_list(sb, cpu, socket);
	spin_lock(&free_list->s_lock);
	...
	if (not_enough_blocks(free_list, num_blocks)) {
		...
		spin_unlock(&free_list->s_lock);
		cpu = odinfs_get_candidate_free_list(sb, socket);
		retried++;
		goto retry;
	}

alloc:
	ret_blocks = odinfs_alloc_blocks_in_free_list(sb, free_list, num_blocks,
						      &new_blocknr);

	spin_unlock(&free_list->s_lock);

	if (ret_blocks <= 0 || new_blocknr == 0) {
		odinfs_dbg("%s: not able to allocate %lu blocks. "
	...
	if (zero) {
		...
		bp = odinfs_get_virt_addr_from_offset(
			sb, odinfs_get_block_off(sb, new_blocknr, 0));

#if ODINFS_DELEGATION_ENABLE
		if (sbi->delegation_ready &&
		    PAGE_SIZE * ret_blocks > ODINFS_WRITE_DELEGATION_LIMIT) {
			memset(odinfs_issued_cnt, 0,
			       sizeof(long) * ODINFS_MAX_SOCKET);
			memset(odinfs_completed_cnt, 0,
			       sizeof(struct odinfs_notifyer) *
				       ODINFS_MAX_SOCKET);

			odinfs_do_write_delegation(sbi, NULL, 0,
						   (unsigned long)bp,
						   PAGE_SIZE * ret_blocks, 1, 1,
						   0, odinfs_issued_cnt,
						   odinfs_completed_cnt, 0);

			odinfs_complete_delegation(odinfs_issued_cnt,
						   odinfs_completed_cnt);
		} else {
			odinfs_memunlock_range(sb, bp, PAGE_SIZE * ret_blocks);
			memset_nt(bp, 0, PAGE_SIZE * ret_blocks);
			odinfs_memlock_range(sb, bp, PAGE_SIZE * ret_blocks);
	...
	*blocknr = new_blocknr;
	...
	odinfs_dbg_alloc("Alloc %lu NVMM blocks 0x%lx at cpu: %d, socket: %d\n",
...
static int odinfs_do_new_data_blocks(struct super_block *sb,
				     ...
				     int zero, int curr_cpu)
{
	...
	int num_blocks = num * odinfs_get_numblocks(inode->i_blk_type);

	allocated = odinfs_new_blocks(sb, blocknr, num_blocks, zero, curr_cpu,
				      inode->nsocket);

	inode->nsocket = odinfs_get_nsocket(sb, inode);
 ...
 * blocknr.  Return the number of blocks allocated.
 */
static int odinfs_do_new_index_blocks(struct super_block *sb,
				      struct odinfs_inode *inode,
				      unsigned long *blocknr, unsigned int num,
				      int zero, int curr_cpu)
{
   ...
   */
	...
	allocated = odinfs_new_blocks(sb, blocknr, num, zero, curr_cpu, socket);
			   ...
			   unsigned long *blocknr, int zero)
{
	...
	int allocated = odinfs_do_new_data_blocks(sb, pi, blocknr, 1, zero,
						  smp_processor_id());
...
int odinfs_new_index_blocks(struct super_block *sb, struct odinfs_inode *inode,
			    unsigned long *blocknr, int zero)
{
	int allocated = odinfs_do_new_index_blocks(sb, inode, blocknr, 1, zero,
						   smp_processor_id());
