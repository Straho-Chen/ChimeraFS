...
static int pmem_ar_create(unsigned long arg) {
  ...
  for (i = 0; i < pmem_arg_info.num; i++) {
    if (strncpy_from_user(path, pmem_arg_info.paths[i], PATH_MAX) < 0) {
      ret = -EFAULT;
      goto err_out;
    }

    /* lock the device so that it cannot be read/write/mount by others */
    pmem_ar_dev.bdevs[i] = blkdev_get_by_path(path, FMODE_READ | FMODE_WRITE,
...
void md_submit_bio(struct bio *bio) { bio_io_error(bio); }

int md_poll_bio(struct bio *bio, struct io_comp_batch *iob,
                unsigned int flags) {
  bio_io_error(bio);
...
int sodinfs_pmem_ar_init_block(void) {
  ...
  pmem_ar_dev.gd->queue = blk_alloc_queue(GFP_KERNEL);
