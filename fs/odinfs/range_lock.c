...
struct range_lock *range_lock_init(unsigned long size)
{
	struct range_lock *lock =
		kzalloc(sizeof(struct range_lock), GFP_KERNEL);
	BUG_ON(!lock);

	odinfs_init_range_lock(lock);
...
void range_lock_free(struct range_lock *lock)
{
	...
	if (lock->sg_size == SEGMENT_INIT_COUNT)
		kmem_cache_free(odinfs_rangelock_cachep, lock->sg_table);
	else
		vfree(lock->sg_table);
#else
	vfree(lock->sg_table);
...
rw_range_lock(volatile struct range_lock *lock, long start, long end,
	      bool reader)
{
...
__always_inline void *range_read_lock(volatile struct range_lock *lock,
...
__always_inline void *range_write_lock(volatile struct range_lock *lock,
				       unsigned long start, unsigned long end)
{
