    ...
    for bsz in "${BLK_SIZES[@]}"; do
        for job in "${NUM_JOBS[@]}"; do
            for workload in "${WORKLOADS[@]}"; do
                ...
                if [[ "${fs}" == "nova" ]]; then
                    table_add_row "$TABLE_NAME_NOVA" "$workload-$bsz $total_time $meta_time $data_time"
                elif [[ "${fs}" == "cknova" ]]; then
                    table_add_row "$TABLE_NAME_CKNOVA" "$workload-$bsz $total_time $meta_time $data_time"
                elif [[ "${fs}" == "pmfs" ]]; then
                    table_add_row "$TABLE_NAME_PMFS" "$workload-$bsz $total_time $meta_time $data_time"
                elif [[ "${fs}" == "idel" ]]; then
                    table_add_row "$TABLE_NAME_IDEL" "$workload-$bsz $total_time $meta_time $data_time"
                elif [[ "${fs}" == "odinfs" ]]; then
                    table_add_row "$TABLE_NAME_ODINFS" "$workload-$bsz $total_time $meta_time $data_time"
                else
                    echo "fs: ${fs}"
                fi
