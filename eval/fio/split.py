...
# 数据文件路径
input_file_path = os.path.join(cur_dir, "performance-comparison-table-copy")
out_name=f"performance-comparison-table-odinfs-blksize-{blksize}"
...
with open(input_file_path, 'r') as infile, open(output_file_path, 'w') as outfile:
    for line in infile:
        ...
        if line.strip() and not line.startswith("fs ops filesz blksz numjobs bandwidth(MiB/s)"):
            parts = line.split()
            if len(parts) > 4 and parts[0].startswith("odinfs") and parts[3] == str(size_in_bytes(blksize)):
                outfile.write(line)
